...
class PaymentController extends Controller
{
    ...
    public function index()
    {
        ...
        // Hitung total kuantitas barang yang bukan jasa
        $totalQty = 0;
        $biayakirim = 0;

        foreach ($cartItems as $item) {
            // Ambil produk berdasarkan id item keranjang
            $product = Product::find($item->id);

            // Periksa jenis produk
            if ($product && $product->jenis !== 'jasa') {
                $totalQty += $item->qty;
                $biayakirim += 10000 * $item->qty; // Tarif biaya kirim per unit
            }
        }

        // Kirim data kuantitas, total harga, dan lainnya ke view
        return view('frontend.pages.payment', compact('totalQty', 'biayakirim'));
    }
    ...
    //     dd($paymentMethod, $paymentStatus, $transactionId, $paidAmount, $paidCurrencyName );
    ...
    //         $orderProduct->jenis = $product->jenis;
    ...
    //         $amount .= ($item->qty * $item->price) + ($item->qty * 10000) ;
    //     }

    //     // Calculate shipping fee and coupon discount
    //     $shippingFee = Session::get('shipping_method')['cost'] ?? 0;
    //     $couponDiscount = getCartDiscount(); // Assume this function returns the discount amount

    //     // Calculate final amount

    //     $order->sub_total = $sub_total;
    //     $order->amount = $amount; // Final amount to be paid
    //     $order->currency_name = $setting->currency_name;
    //     $order->currency_icon = $setting->currency_icon;
    //     $order->product_qty = \Cart::content()->count();
    //     $order->payment_method = $paymentMethod;
    ...
    public function storeOrder($paymentMethod, $paymentStatus, $transactionId, $paidAmount, $paidCurrencyName)
    {
        ...
        // Iterate through cart items to calculate totals
        foreach (\Cart::content() as $item) {
            $product = Product::find($item->id);
            ...
            if ($product) {
                ...
                // Determine if product is 'barang'
                if ($product->jenis === 'barang') {
                    $hasBarang = true;
                    $amount += ($item->qty * $item->price) + ($item->qty * 10000);
                } else {
                    $amount += $item->qty * $item->price; // Include price only for non-'barang' products
                }
            }
        }

        // Set final amount based on presence of 'barang' products
        if ($hasBarang) {
            $order->amount = $amount; // Use calculated amount including shipping fee
        } else {
            $order->amount = $sub_total; // Use subtotal if only 'jasa' products
        }
        ...
        $transaction->transaction_id = $transactionId;
        $transaction->payment_method = $paymentMethod;
        $transaction->amount = $order->amount; // Ensure this uses the final computed amount
        $transaction->amount_real_currency = $paidAmount;
        $transaction->amount_real_currency_name = $paidCurrencyName;
        $transaction->save();
    }
    ...
    public function payWithBank(Request $request)
    {
        ...
        $this->storeOrder('Bank', 0, \Str::random(10), $payableAmount, $setting->currency_name);
        // clear session
        $this->clearSession();
